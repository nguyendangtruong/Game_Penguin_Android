    ...
    public class CharacterMover : ICharacterMover
    {
        ...
        #region copy and set điều kiện khi di chuyển sang trái, phải
        if (m_Character.CharacterInput.Horizontal != 0)
        {
            var movementInput1 = m_Character.IsDead
               ? Vector3.zero
               : new Vector3(m_Character.CharacterInput.Horizontal , 0, m_Character.CharacterInput.Vertical);

            var movementRelativeCamera1 = m_Character.CharacterCamera.transform.TransformDirection(movementInput1);
            movementRelativeCamera1 = new Vector3(movementRelativeCamera1.x , 0, movementRelativeCamera1.z).normalized;

            m_CharacterInputMovement = (m_Character.Speed * movementRelativeCamera1)/2;

            Vector3 externalMovement1 = Vector3.zero;
            for (int i = m_ExternalMovers.Count - 1; i >= 0; i--)
            {
                externalMovement1 += m_ExternalMovers[i].Movement;
                m_ExternalMovers[i].Tick();
            }

            m_Movement = m_CharacterInputMovement + externalMovement1 + m_GravityMovement;
            m_CharacterController.Move(m_Movement * Time.deltaTime*2);
        }
